 ...
 */
export function convertLonLatToVector(
    longitude: number,
    latitude: number,
 ...
 */
export function convertVectorToLonLat(coordinates: AeroflyVector3Float): {
    longitude: number;
    latitude: number;
    altitude_meter: number;
} {
 ...
 */
export function convertHeadingToOrientation(
    ...
    const upRaw = new AeroflyVector3Float(
        position.x,
        position.y,
        position.z / (1.0 - 0.00669437999014), // WGS84
    );
    ...
    const forward = new AeroflyVector3Float(
        east.x * sinTheta + north.x * cosTheta,
        east.y * sinTheta + north.y * cosTheta,
        east.z * sinTheta + north.z * cosTheta,
    ).normalize();
 ...
 * @see
 */
export function convertOrientationToHeading(orientation: AeroflyMatrix3Float, position: AeroflyVector3Float): number {
 ...
 * @see
 */
export function convertDirectionToHeading(direction: AeroflyVector3Float, position: AeroflyVector3Float): number {
    const east = new AeroflyVector3Float(-position.y, position.x, 0).normalize();
 ...
 * @returns {number} feet
 */
export function convertMeterToFeet(meter: number): number {
    return meter / 0.3048;
}
 ...
 */
export function convertFeetToMeter(feet: number): number {
    return feet * 0.3048;
}
 ...
 */
export function convertLbToKg(lb: number) {
